...
5. **Claude publishes it** and gives you a private link. Add it to your phone's home
   screen so it's one tap away before every trip — see "Add it to your phone's home
   screen" below for step-by-step iPhone and Android instructions.
...
4. Give it a short name — "Packing 🎒", "Staging Room", emoji welcome — and tap **Add**.
...
- The icon is a bookmark that looks like an app, not an offline app — opening it needs an
  internet connection and your claude.ai login. (Your tick marks still live only on your
  own phone, as described below.)
...
5. **Claude 幫你發布**，給你一個私人連結。把它加到手機主畫面，出發前一鍵打開──iPhone 和 Android 的詳細步驟見下方「加到手機主畫面」。
...
不用上架 App Store、也不用安裝任何東西──這個工具本身就是一個私人網頁，把它「加入主畫面」之後，就會像一般 App 一樣出現在手機桌面上，打包時一鍵就能打開：
...
3. 往下捲，點**「加入主畫面」**。
4. 取個簡短的名字──「Packing 🎒」、「打包清單」、放 emoji 都可以──然後按**「加入」**。
...
- 這個圖示是「長得像 App 的書籤」，不是離線 App──打開它需要網路連線和 claude.ai 的登入狀態。（你的勾選狀態仍然只存在自己的手機上，見下方說明。）
