...
No App Store, nothing to install: each tool is published as a private webpage, so on your
phone you just open the link and **Add to Home Screen**. The bookmark then sits next to
your real apps, one tap away:

<img src="homescreen.svg" alt="Illustration: two ec-vibes tools saved as icons on a phone home screen" width="450" />

Each skill's README has step-by-step instructions for both iPhone and Android.
...
不用上架 App Store、也不用安裝任何東西：每個工具發布出來就是一個私人網頁，在手機上打開連結、選「**加入主畫面**」，它就會像一般 App 一樣出現在桌面上，一鍵打開：

<img src="homescreen.svg" alt="示意圖：兩個 ec-vibes 工具以圖示形式放在手機主畫面上" width="450" />

iPhone 和 Android 的詳細步驟，請見各 skill 的 README。
